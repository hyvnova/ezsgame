...
def outline(
    ...
    pos = (
        obj_pos[0] - (size[0] - obj_size[0]) / 2,
        obj_pos[1] - (size[1] - obj_size[1]) / 2,
    )
...
def is_colliding(obj1, obj2, draw_collision_box=False, scale=1):
    ...
    if (
        x1 < x2 + w2
        and x1 + w1 > x2  # Check horizontal overlap
        and y1 < y2 + h2
        and y1 + h1 > y2
...
def is_hovering(obj):
    ...
    if (
        mouse_pos[0] >= box[0][0]  # Left side of the box
        and mouse_pos[0] <= box[1][0]  # Right side of the box
        and mouse_pos[1] >= box[0][1]  # Top side of the box
        and mouse_pos[1] <= box[1][1]  # Bottom side of the box
    ):
        return True
    else:
        print(
            f"Mouse pos: {mouse_pos}, Box Top Left: {box[0]}, Box Bottom Right: {box[1]}"
        )
        return False